...
from utils import transform_label_CT_MRI, train_transforms, adjust_learning_rate, set_requires_grad, MI_loss, plot_fn, set_up_logger, deactivate_batchnorm, mix_out, get_blocks_to_be_concat
import monai
...
if not os.path.exists(os.path.join(args.results_folder_name, 'abdominal')):
    os.mkdir(os.path.join(args.results_folder_name, 'abdominal'))
args.results_folder_name = os.path.join(args.results_folder_name, 'abdominal', str(args.seed)+('EMP_' if not args.GIN else ('GIN_' + str(args.GIN_ch) + '_'))+ ('noise_' if args.noise else '')
