...
from utils import prostate_transforms, adjust_learning_rate, set_requires_grad, MI_loss, plot_fn, set_up_logger, deactivate_batchnorm, mix_out, get_blocks_to_be_concat
import monai
...
parser.add_argument("--train_dataset", type=str, default="BMC", help='name of training dataset')
parser.add_argument("--all_dataset", type=str, default="BIDMC,BMC,HK,I2CVB,RUNMC,UCL", help='name of training dataset')
...
parser.add_argument("--lr", type=float, default=0.0004, help="batch size of the forward pass")
...
parser.add_argument("--MI_weight", type=float, default=0.0, help="MI_weight")
...
if not os.path.exists(os.path.join(args.results_folder_name, 'prostate', str(args.seed)+args.train_dataset)):
    os.mkdir(os.path.join(args.results_folder_name, 'prostate', str(args.seed)+args.train_dataset))

args.results_folder_name = os.path.join(args.results_folder_name, 'prostate', str(args.seed)+args.train_dataset, ('EMP_' if not args.GIN else ('GIN_' + str(args.GIN_ch) + '_'))+ ('noise_' if args.noise else '')
...
eval = np.random.choice(l, int(l*0.3), replace=False)
...
train_transform = prostate_transforms['Train']
train_eval_transform = prostate_transforms['Test']
...
######testing loader
test_loaders = []
test_datasets = (args.all_dataset).split(',')
test_datasets.remove(args.train_dataset)

for test_dataset in test_datasets:
    testing_file = open(os.path.join('data_lists', test_dataset), 'r')
    test_lists = json.load(testing_file)
    print(len(test_lists))
    test_transform = prostate_transforms['Test']
...
class Seg_model(nn.Module):
    def __init__(self, args):
        ...
        self.model = get_efficientunet_b2(out_channels=args.num_classes, concat_input=True, pretrained=True)
...
for epoch in pbar:
    ...
    for i, batch_data in pbar_b:
        ...
        if not args.GIN:
            ...
            if args.adv:
                aug_img1_, aug_img2_ = GIN(img)
            else:
                with torch.no_grad():
                    aug_img1_, aug_img2_ = GIN(img)

            aug_img1, aug_img2 = mix_out(img, aug_img1_, aug_img2_)
            ...
            if args.GIN and args.adv:
                loss_MI = (MI_loss(img, aug_img1_, [netG, netF], args) + MI_loss(img, aug_img1_, [netG, netF],
                                                                                 args)) / 2.0
                if flag_MI:
                    print('init F')
                    netF = netF.cuda()
                    ...
                    opt_MI.zero_grad()
                    flag_MI = False
                    loss_MI = MI_loss(aug_img1_, aug_img1_, [netG, netF], args)
    ...
    if epoch % 5 == 0 and epoch != 0:
        with torch.no_grad():
            ...
            for test_loader in test_loaders:
                ...
                test_dice_scores.append(test_dice_score.mean(axis=0))

            test_dice_scores = np.asarray(test_dice_scores)
            logger.info('Test: ')
            logger.info(test_datasets)
            logger.info(test_dice_scores)

            if best_eval < eval_dice_score.mean():
                best_eval = eval_dice_score.mean()
                best_epoch = epoch
                best_test_score = test_dice_scores
